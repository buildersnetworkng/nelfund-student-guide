    ...
    },
    {
      ...
      icon: (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden>
          ...
          <path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h0a2 2 0 012 2v1" />
        </svg>
      ),
    },
    {
...
export default function ShareGuide({ variant = 'button', className = '' }: ShareGuideProps) {
  ...
  const trigger =
    variant === 'icon' ? (
      ...
      >
        <ShareIcon />
        <span className="text-[11px] font-semibold sm:text-xs">Share</span>
      </button>
    ) : variant === 'hero' ? (
      <button
        type="button"
              ...
              >
                ...
                <div className="mt-4 grid grid-cols-3 gap-2.5">
                  {channels.map((ch) => (
                    ...
                    >
                      <span className={`flex h-11 w-11 items-center justify-center rounded-2xl ${ch.tile}`}>{ch.icon}</span>
