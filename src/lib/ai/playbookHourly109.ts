...
import { playbookHourly108 } from './playbookHourly108'
...
/**
 * Hour-109 angle replies for pending-status (largest leftover after other).
 * Same intent, different wording: why-wait vs how-to-check vs what-next vs money-not-in.
 */
export function playbookHourly109(intent: IntentId, userText?: string): string | null {
  const t = userText || ''
  ...
  if (intent === 'official-sources') {
    if (/i\s*dey\s*new|first\s*timer|where\s*i\s*(go|suppose)\s*begin|wetin\s*to\s*(type|ask)|brief\s*me|talk\s*to\s*me\s*small/i.test(t)) {
  ...
  return playbookHourly108(intent, userText)
}
