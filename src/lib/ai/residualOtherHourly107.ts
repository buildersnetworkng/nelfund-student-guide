...
/**
 * Hour-107 leftover catcher for residual unknown "other".
 * Live 2026-09-22: unknownAi 438, other 324, pending-status 70, jamb 40.
 * Focus: Pidgin + vague help fragments/typos, leftover pending / jamb / school / login / repay.
 */
export function residualOtherHourly107(text: string, entities: string[]): IntentResult | null {
  const q = (text || '').trim()
  if (!q) return null
  ...
  if (
    /^(pls|plz|abeg|jare|jo|please)?\s*(help|halp|asist|assist|guyd|guide)\s*(me|us|am)?\s*(ooo+|oo+|abeg|pls|plz|please|small|jare|jo)?\.?$/i.test(q) ||
    /una\s*(fit|gree|dey)\s*(help|assist|guide|show)\s*(me|us|am)?/i.test(q) ||
    /i\s*(just\s*)?(need|nid|wan|want|dey\s*need)\s*(una\s*|your\s*)?(help|assistance|guide)/i.test(q) ||
    /wetin\s*(i|una)\s*(suppose|supose|go|fit)\s*(do|ask|tok)(\s*(now|here|una|abeg))?/i.test(q) ||
    /i\s*dey\s*(lost|lose|confused|confuzed|stranded|blank|stuck)/i.test(q) ||
    /i\s*no\s*(sabi|saby|know|no)\s*(wetin|where|anytin|anything|how)/i.test(q) ||
    /make\s*una\s*(guide|help|show|orientate|explain)\s*me/i.test(q) ||
    ...
    return hit('official-sources', 0.9, ['other', 'greeting-vague'], 'Vague help / short menu 107', 'exploring', entities)
  }
  ...
  if (
    ...
    return hit('pending-application', 0.94, ['pending-status'], 'Pending leftover 107', 'waiting', entities, true)
  }
  ...
  if (
    ...
    return hit('jamb-verification', 0.94, ['jamb'], 'JAMB leftover 107', 'applying', entities, true)
  }
  ...
  if (
    ...
    return hit('school-not-found', 0.93, ['school-list'], 'School list leftover 107', 'applying', entities, true)
  }
  ...
  if (
    ...
    return hit('portal-login', 0.93, ['login'], 'Email / last-year register 107', 'applying', entities, true)
  }
  ...
  if (
    ...
    return hit('repayment', 0.92, ['repayment'], 'Repayment leftover 107', 'repaying', entities)
  }
