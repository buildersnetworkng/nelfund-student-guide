...
export function isOffTopic(text: string, priorIntent?: string | null): boolean {
  ...
  if (/how\s*far|money\s*never|never\s*enter|pending|under\s*review|wetin\s*next|so\s*wetin|approved\s*but|dem\s*never\s*pay|nothing\s*don\s*drop|mates\s*don\s*collect|dashboard\s*(zero|0)/i.test(t)) return false
  if (
