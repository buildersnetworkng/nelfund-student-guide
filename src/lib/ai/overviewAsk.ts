...
export function isOverviewAsk(text: string): boolean {
  const t = (text || '').trim()
  if (!t) return false
  return /how\s+(does\s+)?(nelfund|it|this|the\s+loan|dis)\s+work|how\s+nelfund\s+works|how\s+does\s+this\s+nelfund|how\s+(does\s+|e\s+)?(this|dis)\s+nelfund\s+(thing|stuff|matter|loan)?.{0,20}(dey\s+)?work|nelfund\s+(thing|stuff).{0,20}(dey\s+)?work|how\s+(dis|this)\s+nelfund.{0,30}(dey\s+)?work|how.{0,50}nelfund.{0,40}(dey\s+)?work|nelfund.{0,25}dey\s+work|go\s*through\s+(of\s+)?(the\s+)?(whole\s+)?nelfund|whole\s+nelfund\s+(stuff|thing|process|system)|everything\s+(on|about|on\s+how)\s+(how\s+)?nelfund|know\s+everything\s+(on|about)\s+(how\s+)?nelfund|full\s+(guide|overview|explanation|walk\s*through|walkthrough)|explain\s+(how\s+)?nelfund|tell\s+me\s+(how\s+)?nelfund|overview\s+(of\s+)?nelfund|walk\s*me\s+through\s+(nelfund|the\s+loan)|brief\s+(on|about)\s+nelfund|how\s+the\s+(scheme|loan)\s+works|abeg\s+(yarn|explain)\s+(nelfund|am)\s*(small)?|wetin\s+be\s+(this|dis)\s+nelfund\s+(thing|stuff)|how\s+e\s+take\s+be\s+(for\s+)?nelfund|give\s+me\s+(the\s+)?(full|whole)\s+(gist|picture)\s+(of\s+)?nelfund|nelfund\s+from\s+start\s+to\s+(finish|end)|how\s+does\s+this\s+nelfund\s+thing\s+work|ok\s+give\s+me\s+a\s+go\s+through|nelfund\s+gist\s+from\s+top\s+to\s+bottom|break\s+nelfund\s+down\s+for\s+me|yarn\s+nelfund\s+for\s+me|how\s+dis\s+nelfund\s+matter\s+take\s+work|abeg\s+explain\s+the\s+whole\s+loan\s+thing|how\s+this\s+nelfund\s+thing\s+work|gist\s+of\s+(this\s+|dis\s+)?nelfund|explain\s+the\s+whole\s+nelfund|nelfund\s+full\s+gist|abeg\s+yarn\s+how\s+nelfund\s+take\s+work|break\s+am\s+down\s+for\s+me\s+nelfund|wetin\s+be\s+nelfund\s+from\s+start/i.test(
    t,
  )
}
