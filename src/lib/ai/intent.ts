...
const RULES: Rule[] = [
  ...
  {
    intent: 'scam-safety',
    re: /scam|fraud|\botp\b|agent.*(pay|money)|pay\s*(an?\s*)?agent|whatsapp.*(pay|money|agent)|make\s*i\s*pay|pay\s*\d+\s*k|pay\s*(them|am|him|her)\s*for\s*nelfund/i,
  ...
  },
  {
    ...
    re: /official\s*email|nelfund\s*email|email\s*of\s*nelfund|support\s*email|contact\s*(nelfund|support)|customer\s*(care|service)|helpline/i,
