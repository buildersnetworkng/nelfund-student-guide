...
export const PURPOSE_RE =
  /what\s*is\s*(this\s+)?nelfund|wetin\s*(be|mean)\s*(this\s+)?nelfund|about\s+(this\s+)?nelfund|tell\s*me\s*(about|everything).{0,40}nelfund|why\s+(was|is|were|dem|they|una|we)\s+.{0,32}(nelfund|nel\s*fund|it|dis|this)?.{0,16}(created|create|establish|started|start|begin|form|set\s*up|make)|why\s+(dem|they|una)\s+(take\s+)?(create|make|start)\s+(nelfund|am|it)|why\s+(they|dem)\s+created\s+nelfund|why\s+nelfund\s+(was|is|dey|come|exist)|why\s+nelfund\b|purpose\s+(of\s+)?nelfund|reason\s+(for|dem|they|una)\s+.{0,24}nelfund|mission\s+of\s+nelfund|nelfund\s+(purpose|mission|aim|objective|mean)|who\s+(created|established|started)\s+nelfund|wetin\s+(make|cause|make\s+una)\s+(dem|them|una)?\s*(create|start)|wetin\s+nelfund\s+dey\s+(do|mean)|how\s+come\s+.{0,20}nelfund|na\s+why\s+.{0,20}nelfund|^nelfund\??$/i
...
const RULES: Rule[] = [
  ...
  { intent: 'what-is-nelfund', re: PURPOSE_RE, problem: 'What NELFUND is / why it was created', stage: 'exploring', troubleshooting: false, topics: ['what is', 'purpose'], weight: 26 },
...
export function classifyIntent(question: string, history?: ConversationTurn[]): IntentResult {
  ...
  if (PURPOSE_RE.test(q) && !/is\s+(nelfund|it|portal)\s+(still\s+)?open|deadline|as\s+of\s+today/i.test(q)) {
    return { intent: 'what-is-nelfund', confidence: 0.93, topics: ['what is', 'purpose'], problem: 'What NELFUND is / why it was created', stage: 'exploring', entities: detectEntities(q), isTroubleshooting: false }
  }
