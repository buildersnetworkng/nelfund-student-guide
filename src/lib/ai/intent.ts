...
function liveOpenRe(): RegExp {
  return /is\s+(nelfund|it|portal|application|loan)\s+(still\s+)?(open|dey\s+open)|deadline|as\s+of\s+today|still\s+accept|can\s+i\s+still\s+apply|dem\s+still\s+dey\s+(collect|accept|open)|una\s+still\s+dey\s+(collect|open)|closing\s+date|opening\s+date/i
}
...
export function classifyIntent(question: string, history?: ConversationTurn[]): IntentResult {
  ...
  if (!raw && !q) {
    return { intent: 'official-sources', confidence: 0.32, topics: ['empty'], problem: 'Empty message', stage: 'exploring', entities: [], isTroubleshooting: false }
  }
  ...
  if (/invalid\s*jamb|jamb.*(invalid|fail|verif|format)|jamb\s*no\s*(gree|work)/i.test(q)) {
  ...
  if (/missing\s*(info|information|data|school)|record\s*not\s*found|school\s*no\s*(dey|show)/i.test(q)) {
