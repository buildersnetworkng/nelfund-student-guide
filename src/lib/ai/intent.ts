...
const RULES: Rule[] = [
  ...
  { intent: 'what-is-nelfund', re: /what\s*is\s*(this\s+)?nelfund|about\s+(this\s+)?nelfund|wetin\s*(be\s*)?(this\s+)?nelfund|tell\s*me\s*(about|everything).{0,40}nelfund|why\s+(was|is|dem|they|una)\s+(nelfund|nel\s*fund|it)\s*(created|create|establish|start|begin|form|set\s*up)|why\s+(was|is)\s+(nelfund|nel\s*fund)\s+(created|started|established)|why\s+dem\s+create\s+nelfund|purpose\s+of\s+nelfund|mission\s+of\s+nelfund|nelfund\s+(purpose|mission|aim|objective)|who\s+(created|established|started)\s+nelfund|wetin\s+(make|cause)\s+(dem|them)\s+create\s+nelfund|^nelfund\??$/i, problem: 'What NELFUND is / why it was created', stage: 'exploring', troubleshooting: false, topics: ['what is', 'purpose'], weight: 24 },
