...
const RULES: Rule[] = [
  ...
  { intent: 'jamb-verification', re: /jamb.*(not|isn'?t|no|keep|reject|invalid|fail|accept|work|verif|profile|format)|(not|isn'?t|no|keep|reject|invalid|fail).*jamb|invalid\s*jamb|verify\s*(my\s*)?jamb|jamb\s*profile|profile\s*verif/i, problem: 'JAMB verification', stage: 'applying', troubleshooting: true, topics: ['jamb'], weight: 13 },
  ...
  { intent: 'pending-application', re: /(?<!pending\s)(?<!total\s)(?<!approved\s)\bpending\b(?!\s*loans)|application\s*(is\s*)?pending|status\s*(is\s*)?pending|under\s*review|still\s*(waiting|processing)|nothing\s*(is\s*)?happening|check\s*(my\s*)?(application\s*)?status|my\s*application\s*status|how\s*far\s*(with\s*)?(my\s*)?(application|loan)|submitted\s*but|on\s*hold|loan\s*status|e\s*no\s*dey\s*move/i, problem: 'Application still pending', stage: 'waiting', troubleshooting: true, topics: ['pending'], weight: 11 },
...
export function classifyIntent(question: string, history?: ConversationTurn[]): IntentResult {
  ...
  const keywordBuckets: Array<{ intent: IntentId; re: RegExp; weight: number; problem: string; stage: StudentStage; troubleshooting: boolean; topics: string[] }> = [
    { intent: 'pending-application', re: /\bpending\b|under\s*review|application\s*status|still\s*(waiting|processing)|how\s*far\s*(with\s*)?(my\s*)?(application|loan)/i, weight: 14, problem: 'Application still pending', stage: 'waiting', troubleshooting: true, topics: ['pending'] },
