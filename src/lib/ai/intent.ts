...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    re: /bank.*(detail|account|info|fail|reject|not)|my\s*bank\s*details|bvn.*(fail|reject|not|verif)|\bbvn\b.*work|BVN not working|changed?\s*(my\s*)?bank|wrong\s*account|account\s*number/i,
  ...
  {
    intent: 'how-to-apply',
    re: /how\s*(do\s*i|to)\s*apply|application\s*steps?|start\s*(my\s*)?application|register\s*(for|on)\s*nelfund|fill\s*(my\s*)?(information|application|form)|new\s*application|how\s*(does|do)\s*(nelfund|it)\s*work|how\s*to\s*register|steps?\s*to\s*apply|create\s*(an?\s*)?account|create\s*(?:[\w']+\s+){0,3}account|account\s*creation|sign\s*up|i\s*want\s*to\s*apply|help\s*me\s*apply|abeg\s*how\s*(i\s*)?(go|to)\s*apply|make\s*una\s*help|frustrate\s*me|help\s*me\s*(abeg|please)|abeg\s*help|wetin\s*be\s*(the\s*)?next\s*step|next\s*step|help\s*with\s*(my\s*)?application/i,
  ...
  {
    intent: 'missing-information',
    re: /information\s*(no|not)\s*(complete|correct)|data\s*(no|not)\s*(complete|match)|portal\s*(dey\s*)?(show|saying)\s*error|e\s*dey\s*show\s*error|error\s*on\s*(the\s*)?portal|verification\s*fail/i,
    problem: 'Portal verification error',
    stage: 'applying',
    troubleshooting: true,
    topics: ['missing'],
    weight: 11,
  ...
  {
    intent: 'loan-or-scholarship',
    re: /is\s*(it|this|nelfund)\s*(a\s*)?(free\s*)?(money|loan|scholarship)|free\s*money|loan\s*or\s*scholarship|is\s*nelfund\s*free/i,
    problem: 'Loan or scholarship',
    stage: 'exploring',
    troubleshooting: false,
    topics: ['loan'],
  ...
  {
    intent: 'eligibility',
    re: /private\s*(university|institution|school)|can\s*private|private\s*.*apply|only\s*public|polytechnic\s*can|college\s*of\s*education/i,
  ...
  {
    intent: 'upkeep',
    re: /when\s*will\s*(the\s*)?(money|upkeep|20k|payment)\s*(enter|come|arrive|pay)|money\s*(enter|no\s*dey|never\s*enter)|dem\s*never\s*pay|have\s*(they|dem)\s*paid|disbursement|when\s*(i\s*)?(go|will)\s*collect/i,
    problem: 'Disbursement / money arrival',
    stage: 'waiting',
    troubleshooting: true,
    topics: ['upkeep'],
    weight: 12,
  ...
  {
    intent: 'repayment',
    re: /finish(ed)?\s*nysc|after\s*nysc|nysc\s*(done|finish|complete)|when\s*(do\s*i|to)\s*repay|start\s*repay/i,
    problem: 'Repayment after NYSC',
    stage: 'repaying',
    troubleshooting: false,
    topics: ['repayment'],
    weight: 12,
  ...
  {
    intent: 'bank-information',
    re: /bvn\s*(reject|fail|invalid|not\s*work|mismatch)|reject.*bvn|bvn.*reject/i,
    problem: 'BVN problem',
    stage: 'applying',
    troubleshooting: true,
    topics: ['bank'],
