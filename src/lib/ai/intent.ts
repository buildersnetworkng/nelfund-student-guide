...
const PURPOSE_PHRASES = [
  ...
  'all about nelfund',
  'help me understand nelfund',
  'help me understand this loan',
  'i wan understand nelfund',
  'make i understand nelfund',
  'understand nelfund',
  'why we get nelfund',
...
export function isPurposeAsk(text: string): boolean {
  ...
  if (liveOpenRe().test(q) && !/why|purpose|wetin|what\s*is|meaning|created|create|establish|mission|aim|introduce|exist|understand/i.test(q)) {
    return false
  }
  ...
  if (
    /\b(why|wetin|purpose|meaning|aim|mission|reason|explain|understand)\b/i.test(q) &&
  ...
  if (
    /\bnelfund\b/i.test(q) &&
    /\b(why|wetin|purpose|meaning|na\s+wetin|for\s+wetin|understand)\b/i.test(q) &&
    !liveOpenRe().test(q) &&
...
export function classifyIntent(question: string, history?: ConversationTurn[]): IntentResult {
  ...
  const waitish =
    /when\s*(will|go)\s*(they|dem|una|nelfund)?\s*(pay|send|disburse)|never\s*(see|enter|collect|receive|pay)|how\s*far|pending|under\s*review|money\s*no\s*(drop|enter)|still\s*waiting|dem\s*never/i.test(
      raw || q,
    )

  if (/\bupkeep\b|monthly\s*allowance|20,?000/i.test(q) && !/school\s*fees|institutional\s*charges|tuition/i.test(q) && !waitish) {
  ...
  if (/school\s*fees|institutional\s*charges|tuition/i.test(q) && !/\bupkeep\b/i.test(q) && !waitish) {
  ...
  if (waitish && /school\s*fees|institutional\s*charges|tuition|upkeep|money|alert|pay/i.test(raw || q) && !liveOpenRe().test(raw)) {
    return { intent: 'pending-application', confidence: 0.9, topics: ['pending-status', 'pending'], problem: 'Waiting for fees or upkeep pay', stage: 'waiting', entities, isTroubleshooting: true }
  }
  if (
  ...
  if (/invalid\s*jamb|jamb\s*invalid|jamb.*(invalid|fail|verif|format)|jamb\s*no\s*(gree|work|dey)|utme\s*(number|verif)|jamb\s*profile|profile\s*verif/i.test(q)) {
  ...
  if (
    /\b(eligib|who\s*can\s*apply|do\s*i\s*qualify|am\s*i\s*eligible|polytechnic|\bpoly\b|\bcoe\b|college\s*of\s*education|\bnd\b|\bhnd\b|vocational|i\s*don\s*graduate|i\s*dey\s*nysc|already\s*graduate)\b/i.test(
      q,
    )
  ) {
  ...
  if (/how\s*many\s*(students?|people|applications?)|total\s*(disburse|loan|beneficiar)|n355|355\s*billion|disbursement\s*report/i.test(raw)) {
    return { intent: 'current-information', confidence: 0.84, topics: ['current', 'disbursement'], problem: 'Programme scale / latest figures', stage: 'exploring', entities, isTroubleshooting: false }
  }
