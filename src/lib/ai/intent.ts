...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    re: /as\s*of\s*today|current\s*(info|information|status|update)|latest\s*(update|news|info|nelfund)|still\s*accepting|is\s*nelfund\s*still|accepting\s*applications|has\s*2026|is\s*(the\s*)?(application|portal|loan\s*portal)\s*(still\s*)?open|can\s*i\s*(still\s*)?apply|don'?t\s*have\s*(a\s*)?bvn|no\s*bvn\s*yet|them\s*never\s*open|dem\s*never\s*open|window\s*(still\s*)?(open|close)|portal\s*(no|not|never)\s*(dey\s*)?(open|work)|when\s*dem\s*(go|will)\s*open|when\s*(will|is|does).{0,40}(expire|close|end|open)|what.?s\s*the\s*latest|any\s*latest\s*update/i,
  ...
  {
    intent: 'portal-login',
    re: /\blogin\b|log\s*in|sign\s*in|forgot\s*(my\s*)?password|reset\s*(my\s*)?password|password\s*(not\s*work|reset|issue)|can.?t\s*(login|log\s*in)|portal\s*link|where\s*(i\s*)?(go|enter|open)\s*(the\s*)?(portal|site)|existing\s*(account|application)|already\s*(have|created)\s*(an?\s*)?account/i,
  ...
  {
    intent: 'eligibility',
    re: /eligib|can\s*i\s*apply|am\s*i\s*(eligible|qualified)|who\s*can\s*apply|cgpa|disqualif|who\s*(cannot|can'?t)\s*apply|ineligib/i,
    problem: 'Eligibility',
    stage: 'exploring',
    troubleshooting: false,
    topics: ['eligibility'],
  ...
  {
    intent: 'scam-safety',
    re: /scam|fraud|\botp\b|agent.*(pay|money)|pay\s*(an?\s*)?agent/i,
  ...
  },
  {
    ...
    re: /what\s*(documents?|do\s*i\s*need)|documents?\s*need|requirements?|what\s*do\s*i\s*need\s*(to\s*)?(apply|upload|submit)/i,
  ...
  },
  {
    ...
    re: /contact\s*(nelfund|support)|customer\s*(care|service)|helpline/i,
