...
export const PURPOSE_RE =
  /what\s*(is|are)\s*(the\s+)?(purpose|aim|point|goal|meaning|reason)\s*(of\s+)?(this\s+)?(nelfund|loan|scheme)|what\s*is\s*(this\s+)?nelfund|what\s+does\s+(nelfund|it|this)\s+do|explain\s+(this\s+)?nelfund|overview\s+of\s+nelfund|origin\s+of\s+nelfund|wetin\s*(be|mean)\s*(this\s+)?nelfund|wetin\s+nelfund\s+(be|mean|dey\s+do|for)|nelfund\s+dey\s+do\s+wetin|about\s+(this\s+)?nelfund|tell\s*me\s*(about|everything).{0,40}nelfund|why\s+(was|is|were|did|do|dem|they|una|we|fg|government)\s+.{0,48}(nelfund|nel\s*fund|it|dis|this)?.{0,24}(created|create|establish|established|started|start|begin|form|formed|set\s*up|make|made)|why\s+(dem|they|una|fg|government)\s+(take\s+)?(create|make|start|form|bring)\s+(nelfund|am|it)|why\s+(they|dem)\s+(created|formed|started)\s+nelfund|why\s+nelfund\s+(was|is|dey|come|exist|existed)|why\s+(was|is)\s+nelfund|why\s+nelfund\b|purpose\s+(of\s+)?(nelfund|the\s+(student\s+)?loan)|reason\s+(for|dem|they|una)\s+.{0,24}nelfund|mission\s+of\s+nelfund|nelfund\s+(purpose|mission|aim|objective|mean|meaning)|who\s+(created|established|started|signed)\s+nelfund|wetin\s+(make|cause|make\s+una)\s+(dem|them|una|government|fg)?\s*(create|start)|how\s+come\s+.{0,28}(nelfund|dis\s+loan|this\s+loan)|na\s+why\s+.{0,20}nelfund|wetin\s+be\s+the\s+(purpose|reason)|why\s+dem\s+create\s+nelfund|why\s+una\s+create|wetin\s+be\s+nelfund|why\s+dem\s+take\s+create|point\s+of\s+nelfund|wetin\s+dem\s+(take\s+)?(create|start|make)\s+(nelfund|am|dis\s+loan)|how\s+nelfund\s+come\s+about|wetin\s+make\s+(government|fg|dem)\s+start|^nelfund\??$|what'?s\s+nelfund(\s+for)?|nelfund\s+for\s+wetin|na\s+wetin\s+(be\s+)?nelfund|why\s+(they|dem|una)\s+bring\s+nelfund|wetin\s+nelfund\s+dey\s+mean|who\s+bring\s+nelfund/i
