...
import { playbookHourly107 } from './playbookHourly107'
...
/**
 * Hour-108 replies. Vague help stays a short menu, never a live status dump.
 */
export function playbookHourly108(intent: IntentId, userText?: string): string | null {
  const t = userText || ''

  if (intent === 'official-sources') {
    if (
      /help|halp|helep|asist|assist|guide|gide|menu|confused|confuzed|lost|stranded|where\s*to\s*start|orientate|direct\s*me|wetin\s*i\s*(suppose|go)\s*do|una\s*fit\s*help|i\s*wan\s*ask|show\s*me\s*road|gimme\s*(menu|options)|i\s*no\s*get\s*direction/i.test(
        t,
      )
    ) {
  ...
  return playbookHourly107(intent, userText)
}
