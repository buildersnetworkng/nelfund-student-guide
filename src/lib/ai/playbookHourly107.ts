...
/**
 * Hour-107 replies. Vague help stays a short menu, never a live status dump.
 */
export function playbookHourly107(intent: IntentId, userText?: string): string | null {
  const t = userText || ''

  if (intent === 'official-sources') {
    if (
      /help|halp|asist|assist|guide|menu|confused|confuzed|lost|stranded|where\s*to\s*start|orientate|direct\s*me|wetin\s*i\s*(suppose|go)\s*do|una\s*fit\s*help|i\s*wan\s*ask/i.test(
        t,
      )
    ) {
  ...
  return playbookHourly106(intent, userText)
}
