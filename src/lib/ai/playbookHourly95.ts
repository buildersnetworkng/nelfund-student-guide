...
/** Hour-95 wording. Vague help returns a short menu, never a live status dump. */
export function playbookHourly95(intent: IntentId, userText?: string): string | null {
  const t = (userText || '').trim()
  if (!t) return null

  if (
    intent === 'official-sources' &&
    /help|assist|guide|orientate|confused|lost|start|menu|wetin\s*i\s*(suppose|go)\s*do|how\s*e\s*take\s*be|una\s*fit/i.test(
      t,
    )
  ) {
  ...
  if (intent === 'pending-application' && /sleep|no\s*gree\s*(move|change)|weeks\s*(don|has)\s*pass|processing\s*since|class\s*don\s*collect/i.test(t)) {
  ...
  if (intent === 'repayment' && /repay|pay\s*back|nysc|gsi/i.test(t)) {
