...
import { residualOtherHourly137 } from './residualOtherHourly137'
import { residualOtherHourly138 } from './residualOtherHourly138'
...
export function residualSoftRoute(q: string, entities: string[]): IntentResult | null {
  ...
  if (
    /^(bros|sis|oga|aunty|sir|ma)?\s*(abeg|pls|biko|please)?\s*(help|assist|guide|yarn|talk|orientate)\s*(me|am|us)?\s*(small|jo|jare|now|abeg|pls)?\.?$|^i\s*(just\s*)?(come|dey)\s*(here|online)\s*(abeg)?$|^wetin\s*una\s*(dey|fit)\s*(do|help)\s*(here|for\s*here)?\??$|^talk\s*to\s*me\s*(abeg|now)?$|^i\s*no\s*know\s*wetin\s*to\s*(type|ask|write)$|^anything\s*una\s*fit\s*do\s*(for\s*)?me\??$|^help\s*me\s*abeg\s*i\s*no\s*sabi$|^una\s*fit\s*yarn\s*me\s*(small|the\s*gist)\??$|^i\s*wan\s*ask\s*question$|^question\s*abeg$|^support\s*abeg$|^customer\s*care$|^i\s*need\s*(info|information|orientation)$|^how\s*una\s*take\s*help\s*students?\??$|^e\s*be\s*like\s*say\s*i\s*lost$|^i\s*dey\s*manage\s*abeg$|^wetin\s*suppose\s*i\s*ask\??$|^i\s*just\s*dey\s*find\s*(my\s*)?way$|^una\s*fit\s*brief\s*me\??$|^how\s*(this|dis)\s*chat\s*take\s*work\??$|^wetin\s*(you|una)\s*cover\s*(here|for\s*here)\??$|^i\s*no\s*get\s*(any\s*)?(specific|clear)\s*question$|^make\s*we\s*start\s*small$|^any\s*gist\s*(for|about)\s*students\??$|^i\s*need\s*direction\s*(abeg|pls|please)?$|^pls\s*orientate\s*me(\s*small)?$|^gimme\s*(small\s*)?(gist|brief)$|^hlp\s*me$|^abeg\s*hlp$|^gude\s*me$|^assistt\s*me$|^help\?+$|^guide\s*(jo|jare|abeg)?\.?$|^info\s*(pls|abeg|please)\.?$|^i\s*dey\s*here\s*abeg$|^wetin\s*una\s*sabi\??$|^make\s*una\s*yarn\s*small$|^who\s*fit\s*help\s*me\s*here\??$|^any\s*help\s*here\??$|^nelfund\s*help\s*(abeg|pls)?$|^i\s*just\s*land$|^una\s*dey\??$|^e\s*no\s*clear$|^make\s*una\s*help$|^small\s*help(\s*abeg)?$|^i\s*lost(\s*abeg)?$|^i\s*need\s*una(\s*support)?$|^biko\s*help$|^start\s*me(\s*abeg)?$|^where\s*i\s*go\s*begin$|^just\s*help(\s*abeg)?$|^una\s*there\??$|^help\s*joor$|^guide\s*joor$|^pls\s*menu$|^show\s*menu$|^wetin\s*i\s*fit\s*ask$|^abeg\s*i\s*wan\s*gist$|^you\s*fit\s*help\s*student\??$|^make\s*una\s*brief\s*me\s*now$|^i\s*no\s*sabi\s*wetin\s*dey\s*go\s*on$|^kindly\s*assist(\s*me)?$|^i\s*need\s*someone\s*to\s*talk\s*to$|^how\s*i\s*take\s*start\s*(this|dis)\s*thing$|^wetin\s*una\s*dey\s*offer\??$|^i\s*just\s*wan\s*enquire$|^na\s*student\s*i\s*be(\s*abeg)?$|^i\s*come\s*check\s*una$|^help\s*me\s*figure\s*this$|^i\s*dey\s*find\s*who\s*go\s*explain$|^please\s*i\s*am\s*new\s*here$|^i\s*need\s*orientation(\s*about\s*this)?$|^wetin\s*i\s*suppose\s*type$|^make\s*you\s*help\s*me\s*start$|^i\s*no\s*get\s*question\s*i\s*just\s*need\s*help$|^can\s*someone\s*guide(\s*me)?$|^i\s*wan\s*enquire\s*about\s*loan\s*generally$|^any\s*tin\s*for\s*me\s*here\??$|^i\s*dey\s*lost\s*small$|^abeg\s*orientate$|^show\s*wetin\s*una\s*fit\s*do$/i.test(
      text,
    )
  ) {
    return {
  ...
  for (const fn of [
    ...
    residualOtherHourly114,
    residualOtherHourly115,
    residualOtherHourly116,
    residualOtherHourly117,
    residualOtherHourly118,
    residualOtherHourly119,
    residualOtherHourly120,
    residualOtherHourly121,
    residualOtherHourly122,
    residualOtherHourly123,
    residualOtherHourly124,
    residualOtherHourly125,
    ...
    residualOtherHourly136,
    residualOtherHourly137,
    residualOtherHourly138,
  ]) {
    try {
