...
export function residualSoftRoute(q: string, entities: string[]): IntentResult | null {
  ...
  if (/youtube|tutorial|walkthrough|demo\s*video|show\s*me\s*(how|am|video)|video\s*(how|guide)/i.test(text) && !liveishOpen(text) && !/\bpending\b|how\s*far|money\s*never/i.test(text)) {
    return hit('how-to-apply', 'Video / tutorial how to apply', 'preparing', ['how-to-apply', 'video'], entities, false, 0.7)
  }
  if (/i\s*don\s*pay\s*(my\s*)?(school\s*)?fees|already\s*paid\s*(my\s*)?(school\s*)?fees|i\s*have\s*(already\s*)?paid\s*(school|tuition|fees)/i.test(text) && !/\bpending\b|how\s*far|money\s*never/i.test(text)) {
    return hit('school-fees', 'Already paid school fees myself', 'exploring', ['fees', 'guidance'], entities, false, 0.72)
  }
  if (/una\s*don\s*pay\s*(unilag|lasu|oou|yabatech|unilorin|uniben|my\s*school)|has\s*nelfund\s*paid\s*(my\s*)?(school|institution)|school\s*(don|has)\s*(collect|receive|see)\s*(the\s*)?(money|loan|fees)/i.test(text) && !liveishOpen(text)) {
    return hit('pending-application', 'Has school been paid yet', 'waiting', ['pending-status', 'institution'], entities, true, 0.72)
  }
  if (/(\bbvn\b|\bnin\b).{0,24}(fail|invalid|not\s*match|no\s*gree|mismatch|reject)|bvn\s*(verification|verif)|nin\s*(verification|verif)/i.test(text) && !/\bjamb\b/i.test(text)) {
    return hit('documents-needed', 'BVN / NIN mismatch', 'preparing', ['documents', 'bvn'], entities, true, 0.72)
  }
  if (/help\s*me|i\s*need\s*help|abeg\s*help|assist\s*me|guide\s*me/i.test(text) && /nelfund|loan|portal|apply|upkeep/i.test(text) && !liveishOpen(text) && !/\bpending\b|how\s*far|money\s*never|\bjamb\b/i.test(text)) {
    return hit('how-to-apply', 'Help me apply / guide me', 'preparing', ['how-to-apply', 'guidance'], entities, false, 0.68)
  }
  if (/which\s*(email|number|phone)|how\s*(do\s*i|i\s*go)\s*(reach|message|write)|open\s*(a\s*)?ticket|esupport/i.test(text)) {
    return hit('contact-support', 'How to reach NELFUND', 'exploring', ['contact'], entities, false, 0.72)
  }
