...
export function residualSoftRoute(q: string, entities: string[]): IntentResult | null {
  ...
  if (
    /how\s*(do\s*i|i\s*go|can\s*i)\s*(know|confirm|see).{0,40}(approv|successful|paid|pay|disburse|school\s*(don|has)\s*(collect|receive))/i.test(text) &&
    !liveishOpen(text)
  ) {
    return hit('pending-application', 'How to confirm approval or school payment', 'waiting', ['pending-status', 'approval'], entities, true, 0.7)
  }
  if (
    /(next|this)\s*(month|batch|cycle|payment|disburse)|second\s*batch|batch\s*(2|two)|dem\s*don\s*start\s*(pay|disburse)|has\s*(nelfund|una|they)\s*start(ed)?\s*(pay|disburse)/i.test(
      text,
    ) && !liveishOpen(text)
  ) {
    return hit('pending-application', 'Batch / next payment wait', 'waiting', ['pending-status', 'batch'], entities, true, 0.7)
  }
  if (
  ...
  if (
    /how\s*(nelfund|e|am)\s*take\s*work|how\s*(does|do)\s*(nelfund|the\s*loan)\s*work|explain\s*how\s*(to\s*)?(use|apply)/i.test(text) &&
    !liveishOpen(text) &&
    !/\bpending\b|how\s*far|money\s*never/i.test(text)
  ) {
    return hit('how-to-apply', 'How the process works / apply path', 'preparing', ['how-to-apply', 'guidance'], entities, false, 0.66)
  }
  if (/nelfund\s*(no|not|never)\s*(dey\s*)?(show|display)\s*(my\s*)?(name|record|application)|my\s*name\s*(no|not)\s*(dey\s*)?show/i.test(text)) {
    return hit('missing-information', 'Name / record not showing', 'applying', ['missing', 'upload'], entities, true, 0.7)
  }
