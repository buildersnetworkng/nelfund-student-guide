...
export function detectEntities(q: string): string[] {
  ...
  const map: [RegExp, string][] = [
    ...
    [/eligib|qualify|cgpa|level|fresher|part.?time/i, 'eligibility'],
...
const SCHOOL_ONLY =
  /\b(unilag|lasu|oou|yabatech|unilorin|uniben|oau|unijos|noun?|futo|abu|unizik|unn|unical|uniport|futa|lautech|tasued|ui\b|unimaid|fuoye|ksusta|delsu|aaua|aau|eksu|ksu|nasarawa|buk|udus|uniosun|mouau|funaab|rivers\s*state|yaba\s*tech|lagos\s*state\s*uni|university\s*of\s*lagos|obafemi\s*awolowo)\b/i
...
export function residualSoftRoute(q: string, entities: string[]): IntentResult | null {
  ...
  const alreadyApplied =
    /i\s*(have\s*)?(already\s*)?(don\s*)?apply|submitted\s*(my\s*)?(application|form)|check\s*(my\s*)?(loan|application)|how\s*far\s*(with\s*)?(my\s*)?(loan|application|nelfund)|i\s*don\s*submit/i.test(
      q,
    )
  const wantContact =
    /ticket|complain|esupport|customer\s*care|phone\s*number|hotline|who\s*do\s*i\s*(call|mail|contact)|how\s*(do\s*i|to)\s*reach|official\s*(mail|email)|helpline/i.test(
      q,
    )
  const schoolOnly =
  ...
  const jambFail = /jamb|utme/.test(lower) && /invalid|fail|verif|no\s*gree|reject|not\s*work|can.?t/i.test(q)
  const repayAsk = /repay|pay\s*back|gsi|scholarship|imprison|jail|when\s*(do\s*i|to)\s*pay/i.test(q)
  const wantApply =
    /how\s*(do\s*i|to|i\s*go)\s*(apply|register)|i\s*(wan|want)\s*(to\s*)?apply|start\s*(my\s*)?application|create\s*(an?\s*)?account|sign\s*up/i.test(
      q,
    )
  const loginAsk = /can.?t\s*(log|sign)\s*in|forgot\s*password|portal\s*(no|not)\s*(open|load|work)|blank\s*page|otp\s*(no|not)/i.test(q)
  const missingAsk = /missing|record\s*not|school\s*(no|not)\s*(show|appear|dey)|not\s*on\s*(the\s*)?list/i.test(q)
  const screenshotAsk = /see\s*(the\s*)?(picture|photo|screenshot|image)|what\s*does\s*this\s*mean|this\s*error/i.test(q)
  const greetingish =
    /^(hi|hello|hey|good\s*(morning|afternoon|evening)|how\s*far|wetin\s*dey|sup|yo|pls|please)[\s,!.]*([a-z].{0,40})?$/i.test(
      compact,
    ) && compact.length < 50
  ...
  if (vagueHelp || (pidginHelp && compact.length < 140)) {
