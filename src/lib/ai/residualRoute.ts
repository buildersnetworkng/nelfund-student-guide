...
export function residualSoftRoute(q: string, entities: string[]): IntentResult | null {
  ...
  // Residual "other": school-name-only dumps
  if (
  ...
  // Amount rumours without inventing figures beyond confirmed upkeep
  if (
  ...
  // Closed / they said portal close rumours
  ...
  // Private / part-time / NOUN leftover eligibility
  ...
  // Residual other: first-time apply, next-step follow-ups, NIN/BVN, documents
  if (
