...
export function routeByKeywords(text: string): string | null {
  ...
  const rules: [RegExp, string][] = [
    ...
    [/what\s*is\s*(this\s+)?nelfund|what'?s\s*(this\s+)?nelfund(\s+\w+){0,4}|what'?s\s*nelfund(\s+all)?\s*about|nelfund\s*(all\s*)?about|all\s*about\s*(this\s+)?nelfund|explain(\s+\w+){0,6}\s+(this\s+)?nelfund|explain\s+everything.{0,40}nelfund|tell\s*me\s*everything.{0,40}nelfund|everything\s+about\s+(this\s+)?nelfund|wetin\s*(be\s*)?(this\s+)?nelfund|about\s+(this\s+)?nelfund|meaning\s*of\s*nelfund|nelfund\s*stand\s*for|help\s*me\s*understand\s*nelfund|understand\s*nelfund|tell\s*me\s*about\s+(this\s+)?nelfund|break\s*down\s*nelfund|overview\s*of\s*nelfund|describe\s+(this\s+)?nelfund|teach\s*me\s*(about\s*)?(this\s+)?nelfund|i\s*want\s*to\s*know\s*(about\s*)?(this\s+)?nelfund/, 'whatis'],
    ...
    [/which\s*(link|url|website|site)|portal\s*link|where\s*(do\s*i|to)\s*(login|log\s*in|loggin'?g\s*in)|login\s*link|loggin'?g\s*in|loggin'?g\s*into|link\s*(for\s*)?(to\s*)?(loggin'?g\s*in|login|log\s*in)|how\s*(do\s*i|to)\s*(login|log\s*in|loggin'?g\s*in)|official\s*(website|site|portal)|how\s*(to|do\s*i)\s*(enter|access)\s*(the\s*)?portal|abeg\s*which\s*site|link\s*to\s*apply|where\s*to\s*register|continue\s*(my\s*)?application/, 'login'],
    [/jamb/, 'jamb'],
