...
import { playbookHourly170 } from './playbookHourly170'
import { playbookHourlyChain } from './playbookHourlyChain'
...
export function playbookAnswer(intent: IntentId, ctx: PlaybookContext): string | null {
  ...
  const chained = playbookHourlyChain(intent, userText)
  if (chained) return chained
  const h170 = playbookHourly170(intent, userText)
  if (h170) return h170
  ...
  if (intent === 'email-already-used') {
    return (
      ...
      'That email may already have an account \u2014 sign in or reset rather than opening a second one.'
    )
  }
  ...
  if (intent === 'pending-application') {
    if (/cancel|dispute|wrong\s*fee|re-?apply/i.test(userText)) {
      return (
        ...
        '4. On a new application, use **Raise a dispute** before Submit if the fee shown is not the bursary figure.\n' +
