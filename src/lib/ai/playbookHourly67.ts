...
/** Hour-67 playbook. Vague help = short menu. No invented dates. No long dashes. */
export function playbookHourly67(intent: string, userText: string): string | null {
  const t = userText || ''

  if (
    /^(abeg\s*)?(help|assist|guide)\s*(me)?|una\s*fit\s*(help|assist)|i\s*(just\s*)?(need|wan|want)\s*help|pls\s*assist|wetin\s*i\s*(suppose|go)\s*do|i\s*dey\s*lost|help\s*abeg|i\s*no\s*know\s*where\s*to\s*start|make\s*una\s*guide\s*me|can\s*you\s*help\s*me|i\s*need\s*assistance/i.test(
      t,
    )
  ) {
