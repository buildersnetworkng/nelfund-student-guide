...
export function playbookPendingExtras(t: string): string | null {
  ...
  if (/how\s*far\s*(my\s*)?(loan|application|nelfund|am|money)|check\s*(my\s*)?(application|loan|status)|track\s*(my\s*)?(application|loan)/i.test(t)) {
  ...
  if (/(mates|class|course\s*mates|everybody|others)\s*(don|have|has)\s*(collect|receive|get|see)|dem\s*don\s*pay\s*(my\s*)?(mates|class|everybody|others)|only\s*me\s*(remain|never\s*collect)/i.test(t)) {
    return `**Mates don collect and you never see money is still your portal file, not a public class list.**\n\n1. Open ${PORTAL} and copy the exact status word. I cannot see your file from this chat.\n2. Institutional charges go to the **school**. Your mates can get upkeep first if they ticked it and their school record cleared earlier.\n3. Do not open a second account because classmates got paid.\n4. I will not invent a batch or pay date. Long same-word wait: campus NELFUND desk, then ${ESUPPORT}.`
  }
  if (/\bbatch\b|una\s*don\s*pay\s*(my\s*)?school|school\s*(don|has)\s*(collect|receive)|dem\s*don\s*pay\s*(the\s*)?school|school\s*fees?\s*(don|has)\s*(enter|pay|paid)/i.test(t)) {
    return `**Batch / school already paid** is still your portal status, not a public list I can invent.\n\n1. Only ${PORTAL} shows *your* file. There is no official public batch sheet I can quote here.\n2. If the school already received institutional charges, upkeep (only if you ticked it) can still be later.\n3. Copy the exact status word from the portal.\n4. Same word for a long time: campus desk, then ${ESUPPORT}. I will not invent a pay date.`
  }
  if (/^(how\s*far|how\s*far\s*na|how\s*far\s*now|abeg\s*how\s*far)[.!? ]*$/i.test(t) || /money\s*(never|no)\s*(enter|drop|show)|e\s*never\s*(enter|drop)|never\s*enter|account\s*(empty|blank|zero)|e\s*never\s*(show|enter|drop)\s*(for|in)\s*(my\s*)?(account|bank)/i.test(t)) {
  ...
  if (/glitch|system\s*(fail|failure|down)|two\s*months?\s*(no|never|without)|upkeep\s*(delay|delayed|hold|on\s*hold)|allowance\s*(delay|delayed)|stipend\s*(never|no)\s*(enter|drop|come)|nans/i.test(t)) {
  ...
  if (/name\s*(no|not)\s*dey\s*(the\s*)?(pay\s*)?list|tinubu\s*(list|pay)|pay\s*list/i.test(t)) {
    return `**Name no dey any public pay list I can invent.**\n\n1. Only ${PORTAL} shows *your* file. Copy the exact status word there.\n2. Institutional charges go to the **school**. No personal alert does not mean declined.\n3. Upkeep only if you ticked it in the same session, and it can land later.\n4. I will not invent a Tinubu list or pay date. Long same-word wait: campus NELFUND desk, then ${ESUPPORT}.`
  }
