...
export function residualOtherRoute(text: string, entities: string[]): IntentResult | null {
  ...
  if (/e\s*no\s*(gree|work|open)|portal\s*(no|not|never)\s*(gree|work|load|open)|error\s*(500|503|404)|screenshot|see\s*(the\s*)?picture|look\s*(this\s*)?error/i.test(q)) {
    return hit('missing-information', 0.72, ['other', 'portal-error'], 'Portal error / screenshot leftover', 'applying', entities, true)
  }
  if (/declin|reject(ed)?|not\s*approv|dem\s*no\s*gree\s*approve/i.test(q) && !liveish(q)) {
    return hit('pending-application', 0.8, ['pending-status', 'declined'], 'Declined / not approved leftover', 'waiting', entities, true)
  }
