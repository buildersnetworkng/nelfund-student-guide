...
export function residualOtherRoute(text: string, entities: string[]): IntentResult | null {
  ...
  if (/how\s*much\s*(have\s*)?(they|dem|una|nelfund)\s*(disburse|pay|paid)|how\s*many\s*(students?|beneficiar)|disburse(d|ment)?\s*(figure|total|amount|so\s*far)|n355|355\s*billion|how\s*far\s*(nelfund|the\s*scheme)\s*(don|has)\s*pay/i.test(q) && !liveish(q)) {
    return hit('official-sources', 0.82, ['other', 'disbursement-totals'], 'Disbursement totals leftover', 'exploring', entities)
  }
  if (/efcc|recovered\s*(loot|fund)|unclaimed\s*dividend|tinubu\s*(order|direct)/i.test(q)) {
    return hit('official-sources', 0.8, ['other', 'funding'], 'EFCC / funding leftover', 'exploring', entities)
  }
  if (/25,?000|increase(d)?\s*(upkeep|allowance|stipend)|upkeep\s*(don|has)\s*(increase|change)|from\s*20,?000/i.test(q)) {
    return hit('upkeep', 0.88, ['upkeep', 'amount-rumour'], 'Upkeep increase rumour leftover', 'exploring', entities)
  }
  if (/still\s*(dey\s*)?pay(ing)?|una\s*still\s*dey\s*pay|dem\s*still\s*dey\s*pay|is\s*nelfund\s*still\s*(paying|disbursing)/i.test(q) && !liveish(q)) {
    return hit('pending-application', 0.8, ['pending-status', 'still-paying'], 'Still paying leftover', 'waiting', entities, true)
  }
  if (/next\s*batch|when\s*(is|be)\s*(the\s*)?next\s*(payment|batch|disburse)/i.test(q) && !liveish(q)) {
    return hit('pending-application', 0.84, ['pending-status', 'next-batch'], 'Next batch leftover', 'waiting', entities, true)
  }
  if (/sustainab|can\s*nelfund\s*(continue|last|survive)|go\s*nelfund\s*collapse|recovery\s*system/i.test(q)) {
    return hit('repayment', 0.78, ['repayment', 'sustain'], 'Scheme sustainability leftover', 'repaying', entities)
  }
  if (/who\s*(qualify|eligible)|who\s*fit\s*apply|who\s*can\s*collect/i.test(q) && !liveish(q)) {
    return hit('eligibility', 0.86, ['eligibility', 'who'], 'Who can apply leftover', 'exploring', entities)
  }
  if (/matric\s*(no|not|never)|invalid\s*matric|matric\s*number\s*(wrong|fail)/i.test(q)) {
    return hit('documents-needed', 0.82, ['documents', 'matric'], 'Matric number leftover', 'preparing', entities, true)
  }
  if (/dashboard\s*(blank|empty)|nothing\s*dey\s*show\s*for\s*(portal|dashboard)|white\s*screen/i.test(q)) {
    return hit('portal-login', 0.82, ['login', 'blank'], 'Blank dashboard leftover', 'applying', entities, true)
  }
  return null
}
