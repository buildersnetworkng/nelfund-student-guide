...
export function recoverFromToolResult(result: ToolResult): RecoveryMessage | null {
  ...
  switch (result.name) {
    case 'get_institution_guidance':
      if (result.status === 'not_found') {
        return {
          message:
            "I couldn't verify that institution in the curated directory. Share the full official name (or short form like LASU, UNILAG). I can still help you draft a message and point you to NELFUND eSupport — I will not invent an email address.",
          actions: [
            `NELFUND support: ${ESUPPORT}`,
            `Portal: ${PORTAL}`,
          ],
  ...
  return {
    message:
      'Something went wrong looking that up. You can still use the official portal and NELFUND eSupport. Tell me what you are trying to do and we can continue safely.',
    actions: [`Portal: ${PORTAL}`, `Support: ${ESUPPORT}`],
...
export function conflictSchoolUploadedStillMissing(institutionName: string | null): RecoveryMessage {
  ...
  return {
    message: [
      ...
      `3. If the school confirms the upload is correct and the portal still fails, open a tracked ticket with NELFUND: ${ESUPPORT}.`,
      '',
