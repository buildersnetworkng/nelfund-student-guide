...
function isVagueHelpAsk(text: string): boolean {
  const t = text.trim().toLowerCase().replace(/[!.,?]+$/g, '').trim()
  if (!t || t.length > 90) return false
  if (/nelfund|portal|pending|jamb|repay|upkeep|deadline|login|sign\s*in|school\s*fees|missing/i.test(t)) return false
  return /^(abeg\s*i\s*wan\s*gist|you\s*fit\s*help\s*student|make\s*una\s*brief\s*me(\s*now)?|i\s*no\s*sabi\s*wetin\s*dey\s*go\s*on|kindly\s*assist(\s*me)?|i\s*need\s*someone\s*to\s*talk\s*to|how\s*i\s*take\s*start\s*(this|dis)\s*thing|wetin\s*una\s*dey\s*offer|i\s*just\s*wan\s*enquire|na\s*student\s*i\s*be(\s*abeg)?|i\s*come\s*check\s*una|help\s*me\s*figure\s*this|i\s*dey\s*find\s*who\s*go\s*explain|please\s*i\s*am\s*new\s*here|i\s*need\s*orientation(\s*about\s*this)?|wetin\s*i\s*suppose\s*type|make\s*you\s*help\s*me\s*start|i\s*no\s*get\s*question\s*i\s*just\s*need\s*help|can\s*someone\s*guide(\s*me)?|i\s*wan\s*enquire\s*about\s*loan\s*generally|any\s*tin\s*for\s*me\s*here|i\s*dey\s*lost\s*small|abeg\s*orientate|show\s*wetin\s*una\s*fit\s*do)$/i.test(t)
}
...
async function processUserTurnInner(opts: {
  ...
  if (!ocr && rawUser && isVagueHelpAsk(rawUser)) {
    return finalize(userMsg, { ...opts.slots }, 'official-sources', greetingReply(), 'conversation')
  }
