...
export default function Ask() {
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col bg-paper">
      <header className="relative z-30 flex h-12 shrink-0 items-center justify-between border-b border-forest-100 bg-paper px-3 sm:h-14 sm:px-5">
        <div className="flex min-w-0 items-center gap-2.5">
          <Link
            to="/"
            className="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-xl shadow-sm"
            aria-label="Back to home"
          >
            <img src="/brand/logo-ai.svg" alt="" width={32} height={32} className="h-8 w-8" />
          </Link>
          <div className="relative min-w-0" ref={schoolMenuRef}>
            <p className="truncate text-sm font-semibold text-ink">NELFUND Support</p>
            <button
              type="button"
            ...
            {showSchoolMenu && (
              <div
                ...
                className="absolute left-0 top-full z-[100] mt-1 max-h-64 w-[min(92vw,18rem)] overflow-y-auto rounded-xl border border-forest-100 bg-white py-1 shadow-xl ring-1 ring-black/10 isolate"
              >
                <button
                  type="button"
                  role="option"
          ...
          <div className="mx-auto flex h-full max-w-2xl flex-col justify-center px-4 py-10 sm:px-6">
            <div className="fade-in text-center">
              <h1 className="font-display text-2xl font-semibold tracking-tight text-ink sm:text-3xl">
                How can NELFUND Support help?
              </h1>
              <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-ink/55">
                Guidance on applications, portal errors, school records, and verified support contacts, based on
                official NELFUND information.
              </p>
            </div>
            {!institutionId && (
              <div className="mx-auto mt-5 max-w-md rounded-2xl border border-gold-300/40 bg-gold-50/80 px-4 py-3 text-left shadow-sm">
                ...
                <p className="mt-1 text-xs leading-relaxed text-ink/55">
                  Choosing your institution improves portal advice and routes you to the right campus desk. Use the{' '}
                  <button
                    type="button"
                    onClick={() => setShowSchoolMenu(true)}
                    className="font-semibold text-forest-800 underline decoration-forest-300 underline-offset-2"
                  >
                    school selector
                  </button>{' '}
                  above, or type your school name in the chat.
                </p>
              </div>
            )}
            {institutionId && institutionId !== 'other' && displaySchoolName && (
              <p className="mx-auto mt-4 max-w-md text-center text-xs text-forest-800/80">
                Helping students at <span className="font-semibold">{displaySchoolName}</span>
              </p>
            )}
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-2 rounded-xl border border-forest-100/80 bg-forest-50/40 px-2.5 py-1.5">
                        ...
                        {!feedback[m.id] && (
                          <>
                            <button
                              ...
                              className="inline-flex items-center gap-1 rounded-full bg-white px-2.5 py-1 text-xs font-semibold text-forest-800 shadow-sm ring-1 ring-forest-100 transition hover:bg-forest-50"
                              aria-label="Helpful"
                            >
                              👍 Yes
                            </button>
                            <button
                              ...
                              className="inline-flex items-center gap-1 rounded-full bg-white px-2.5 py-1 text-xs font-semibold text-ink/70 shadow-sm ring-1 ring-forest-100 transition hover:bg-rust-50 hover:text-rust-700"
                        ...
                        {feedback[m.id] === 'up' && !shareNudgeDismissed && (
                          <div className="mt-1 flex w-full flex-wrap items-center gap-2">
                            <span className="text-[11px] text-ink/55">Know another student who needs this?</span>
      ...
      <div className="relative z-10 shrink-0 border-t border-forest-100 bg-paper px-3 py-3 sm:px-5">
        <div className="mx-auto max-w-2xl">
          {pendingPreview && (
            <div className="mb-2 flex items-center gap-2 rounded-xl border border-forest-100 bg-white p-2 shadow-sm">
              <img src={pendingPreview} alt="Pending" className="h-12 w-12 rounded-lg object-cover" />
              <p className="min-w-0 flex-1 truncate text-xs text-ink/60">Screenshot ready to send</p>
              <button type="button" onClick={clearPendingImage} className="rounded-full px-2 py-1 text-xs font-semibold text-rust-700">
                Remove
              </button>
            </div>
          )}
          <form onSubmit={onSubmit} className="flex items-end gap-2">
            <div className="relative">
              <button
                ...
                className="flex h-11 w-11 items-center justify-center rounded-full border border-forest-100 bg-white text-lg text-forest-800 shadow-sm"
                aria-label="Attach screenshot"
              >
                +
              </button>
              {showAttachMenu && (
                <div className="absolute bottom-full left-0 mb-2 w-40 overflow-hidden rounded-xl border border-forest-100 bg-white shadow-xl">
                  <button type="button" className="block w-full px-3 py-2 text-left text-xs hover:bg-forest-50" onClick={() => galleryRef.current?.click()}>
                    Gallery
                  </button>
                  <button type="button" className="block w-full px-3 py-2 text-left text-xs hover:bg-forest-50" onClick={() => cameraRef.current?.click()}>
                    Camera
                  </button>
                </div>
              )}
            ...
            <textarea
              ...
              className="max-h-[140px] min-h-[44px] flex-1 resize-none rounded-2xl border border-forest-100 bg-white px-3.5 py-2.5 text-sm text-ink shadow-sm outline-none focus:border-forest-300"
              onKeyDown={(e) => {
            ...
            <button
              ...
              className="flex h-11 w-11 items-center justify-center rounded-full bg-forest-700 text-paper shadow-sm disabled:opacity-40"
              aria-label="Send"
            >
              ↑
            </button>
