        ...
        <div className="min-w-0 flex-1 space-y-3 pb-32">
          ...
          {messages.map((m) => (
            <div key={m.id} className="space-y-1.5">
              {m.role === 'user' ? (
                <div className="ml-auto max-w-[92%] rounded-2xl rounded-br-md bg-green-800 px-3.5 py-2.5 text-sm text-white shadow-md lg:max-w-[70%]">
                  {m.imagePreview && (
                    <img
                      src={m.imagePreview}
                      alt="Uploaded"
                  ...
                  <p className="whitespace-pre-wrap leading-relaxed text-white">{m.text || '…'}</p>
                </div>
              ) : (
                <>
