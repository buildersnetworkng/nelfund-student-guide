...
const ocrCases = [
  {
    ...
    ocr: 'Invalid jamb number format, e.g 0000 00AA\nJamb Profile Verification\nEnter JAMB Registration No.',
    want: /Invalid JAMB number format/i,
    wantSupport: /Report the problem to|report to NELFUND|Report issues to NELFUND/i,
  },
  {
    name: 'ocr-invalid-jamb-noise',
    ocr: 'lnvalid jamb number format e g 0000 00AA\nJamb Profile Verification',
    want: /Invalid JAMB number format/i,
  },
  {
    name: 'ocr-jamb-form-clean',
    ocr: 'Jamb Profile Verification\nEnter JAMB Registration No.\nDate of Birth\nVerify JAMB Profile',
    want: /JAMB Profile Verification|Registration Number/i,
  },
  {
    name: 'ocr-login',
    ocr: 'NELFUND\nKindly provide the required details to login\nEmail\nPassword\nLog In\nReset password\nCreate New Account',
    want: /login|Log In|password/i,
  },
  {
    name: 'ocr-dashboard-zeros',
    ocr: 'Welcome to Student Loan Portal\nNotice 2025/2026 Session Registration\nStarts March 5th 2026 And Ends June 5th 2026\nTotal Loans 0\nApproved Loans 0\nPending Loans 0\nDeclined Loans 0',
    want: /successfully signed in|dashboard|Total|0/i,
  },
  {
    name: 'ocr-eligibility-citizen',
    ocr: 'Get started with answering these questions\nAre you a Nigerian?\nYes, I am a Nigerian\nNo, I am not',
    want: /Citizenship|Nigerian|apply|eligibility/i,
  },
  {
    name: 'ocr-missing-info',
    ocr: 'Missing Information\nStudent record not found\nUnable to verify',
    want: /Missing information|school|ICT|Registry/i,
  },
  {
    name: 'ocr-portal-landing',
    ocr: 'Welcome to the Student Loan Portal\nInterest free\nFast & Easy\nSafe & Secure\nApply Now',
    want: /portal|sign up|apply|account/i,
  },
]
...
for (const c of ocrCases) {
  ...
  if (u) {
    check(`${c.name}-no-bold-url`, !hasBrokenBoldUrl(u.explanation))
    check(`${c.name}-support-lang`, supportLanguageOk(u.explanation))
  }
  ...
  if (c.wantSupport) {
    check(`${c.name}-report-phrase`, c.wantSupport.test(text) || c.wantSupport.test(u?.explanation || ''))
  }
}
...

{
  ...
  check(
    'multi-jamb-then-school',
    /LASU|Lagos State|Invalid JAMB|support|ICT|Registry/i.test(r2.text),
    r2.text.slice(0, 150),
  )
}

const intentCases = [
  ['where do I login', 'portal-login'],
  ['official portal link', 'portal-login'],
  ['someone asked me to pay agent for NELFUND', 'scam-safety'],
  ['they want my OTP for NELFUND', 'scam-safety'],
  ['am I eligible', 'eligibility'],
  ['who can apply for NELFUND', 'eligibility'],
  ['As an 100 level, can I apply for nelfund', 'eligibility'],
  ["I'm a 200 level student", 'eligibility'],
  ...
  ['Draft me an email about missing information', 'email-draft'],
  ['wetin be upkeep for NELFUND', 'upkeep'],
  ['do I need to repay the loan', 'repayment'],
  ['my school data no upload yet', 'institution-verification'],
  ['portal shows missing information', 'missing-information'],
...
for (const q of ["I'm a 200 level student", '100 level can i apply', 'who can apply', 'how do I login']) {
  const a = playbookAnswer(classifyIntent(q).intent, {
...
for (const [user, ocr] of combos) {
  ...
  check(
    `combo-${user || 'ocr-only'}-${ocr.slice(0, 20).replace(/\s+/g, '_')}`,
    text.length > 40 && supportLanguageOk(text) && !hasBrokenBoldUrl(text),
    text.slice(0, 80),
  )
}
...
try {
  ...
  check('architecture', false, e.message)
}
